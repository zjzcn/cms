...
public class WhereCondition implements QueryCondition {
	...
	public WhereCondition(String propertyName, Object value1, Object value2,
			Op op) {
	...
	public String toQueryString() {
		...
		case BETWEEN: {
			hqlBuf.append(propertyName).append(
					" between ?" + paramList.size() + " and ?"
							+ (paramList.size() + 1));
			paramList.add(value1);
		...
		case NOT_IN: {
			...
			hqlBuf.append(propertyName).append(" not in (").append(buf)
					.append(')');
			break;
		}
		case IS_NULL: {
